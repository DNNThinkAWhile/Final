...
FEAT_NUM = 69 + 39
INPUT_FILE = 'MLDS_HW1_RELEASE_v1/mfccfbank/test.normalized.ark'
BATCH = 256
...
def main(argv):
    ...
    (spids, xs)  = read_data(INPUT_FILE, count=180406)
    #ys = [int(spchid_phone_map[spid]) for spid in spids]
    sum_pred = 0
    sum_correct = 0
    for i in xrange(len(xs) / BATCH + 1):
        ...
        print [prob.argmax() for prob in pred]
